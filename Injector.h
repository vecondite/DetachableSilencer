...
inline bool game_version_manager::Detect()
{
    ...
    switch(base + nt->OptionalHeader.AddressOfEntryPoint)
    {
        ...
        case 0x824570:  // GTA SA 1.0 US Compact
            ...
            cracker = linkInjector::ReadMemory<uint8_t>(raw_ptr(0x406A20), true) == 0xE9? 'H' : 0;
            return true;

        case 0x8245BC:  // GTA SA 1.0 EU Cracked (??????)
        case 0x8245B0:  // GTA SA 1.0 EU Cracked
            game = 'S', major = 1, minor = 0, region = 'E', steam = false;
            cracker = linkInjector::ReadMemory<uint8_t>(raw_ptr(0x406A20), true) == 0xE9? 'H' : 0;  // just to say 'securom'
            return true;
